...
func (r *userGormRepo) FindByUsername(ctx context.Context, username string) (*domain.User, error) {
	var u domain.User
	if err := r.db.WithContext(ctx).Preload("Role.Permissions").Where("username = ?", username).First(&u).Error; err != nil {
		return nil, err
	}
	return &u, nil
}
...
func (r *userGormRepo) FindByID(ctx context.Context, id uint) (*domain.User, error) {
	var u domain.User
	if err := r.db.WithContext(ctx).Preload("Role.Permissions").First(&u, id).Error; err != nil {
		return nil, err
	}
	return &u, nil
}
...
func (r *userGormRepo) List(ctx context.Context) ([]domain.User, error) {
	...
	if err := r.db.WithContext(ctx).Preload("Role.Permissions").Find(&users).Error; err != nil {
		return nil, err
	}
	return users, nil
}
